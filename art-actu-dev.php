...
<title>[DEV] Art &amp; Expos — Rennes &amp; Ille-et-Vilaine</title>
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Full-screen map background: everything else (hero, filters, bottom
     panel) floats on top of it as fixed overlay bars, instead of the map
     being one section in a normally-scrolling page. DEV-only layout. */
  .map-frame { position: fixed; inset: 0; z-index: 0; }
  #map { width: 100%; height: 100%; background: #EDE9E1; }
  .hero { position: fixed; top: 0; left: 0; right: 0; z-index: 20; width: 100%; height: 84px; overflow: hidden; pointer-events: none; background: rgba(11,11,14,.35); }
  .hero canvas { display: block; width: 100%; height: 100%; }
  .updated-below { position: fixed; top: 160px; left: 16px; z-index: 20; margin: 0; padding: 0; font-size: 0.7rem; color: #F4F1EB; white-space: nowrap; text-shadow: 0 1px 4px rgba(0,0,0,.7), 0 0 10px rgba(0,0,0,.5); pointer-events: none; }
  ...
  .legend { position: fixed; top: 90px; left: 16px; z-index: 20; width: fit-content; display: flex; flex-wrap: wrap; align-items: center; gap: 4px; padding: 4px; background: rgba(11,11,14,.55); border-radius: 20px; pointer-events: none; }
  .legend > * { pointer-events: auto; }
  .legend .item { display: inline-flex; align-items: center; justify-content: center; width: 26px; height: 26px; padding: 0; color: #F4F1EB; background: #1A1916; border: 1.5px solid #34322C; border-radius: 50%; font: inherit; cursor: pointer; transition: opacity .15s, border-color .15s, background .15s; }
  .legend .item:hover { border-color: #C1694F; }
  .legend .item[aria-pressed="true"] { border-color: currentColor; background: #262521; }
  .legend .item[aria-pressed="false"] { opacity: .4; background: #1A1916; filter: grayscale(60%); }
  .legend .item-action { color: #0B0B0E; background: #E8C468; border-color: #E8C468; font-weight: 700; font-size: 0.95rem; }
  ...
  .active-filter-label { position: fixed; top: 122px; left: 16px; z-index: 20; display: inline-flex; align-items: center; gap: 6px; padding: 4px 10px 4px 4px; background: rgba(11,11,14,.72); border-radius: 20px; color: #F4F1EB; font-size: 0.75rem; font-weight: 600; pointer-events: none; animation: filterLabelIn .18s ease-out; }
  .active-filter-label .chip { width: 16px; height: 16px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; flex: none; }
  .active-filter-label .chip svg { width: 10px; height: 10px; stroke: #fff; }
  @keyframes filterLabelIn { from { opacity: 0; transform: translateY(-4px); } to { opacity: 1; transform: translateY(0); } }
  .panel-bg { position: fixed; left: 0; right: 0; bottom: 0; z-index: 20; max-height: 180px; overflow-y: auto; }
  .site-footer { text-align: center; padding: 8px 14px; margin: 10px auto 14px; color: #F4F1EB; font-size: 0.78rem; background: rgba(11,11,14,.85); border-radius: 20px; width: fit-content; max-width: calc(100% - 32px); }
  .site-footer a { color: #B7B2A8; }
  .site-footer a:hover { color: #E8C468; }
  .panel { padding: 12px 22px 14px; max-width: 900px; margin: 0 auto; }
  .panel h2 { font-size: 1rem; margin: 10px 0 12px; font-weight: 600; color: #F4F1EB; text-shadow: 0 1px 4px rgba(0,0,0,.7), 0 0 12px rgba(0,0,0,.5); }
  .panel .empty { color: #F4F1EB; text-shadow: 0 1px 4px rgba(0,0,0,.7), 0 0 12px rgba(0,0,0,.5); }
...
<div class="hero" style="position:relative;">
  ...
  <span style="position:absolute;top:8px;left:8px;background:#E8C468;color:#2B2E33;font-weight:700;font-size:0.72rem;letter-spacing:.05em;padding:3px 8px;border-radius:4px;z-index:2;">DEV</span>
</div>

<div class="legend" id="legend">
<?php foreach (CATEGORIES as $key => $cat): ?>
  <button type="button" class="item" data-category="<?= $key ?>" aria-pressed="true" title="<?= htmlspecialchars($cat['label']) ?>" aria-label="<?= htmlspecialchars($cat['label']) ?>">
...
<?php endforeach; ?>
  <button type="button" class="item item-action" id="filter-show-all" title="Tout afficher" aria-label="Tout afficher">↻</button>
</div>
<div id="active-filter-label" class="active-filter-label" hidden></div>
...
function applyFilter() {
  ...
  updateActiveFilterLabel(active);
}

// Shows which single category is currently isolated (hidden again once
// every category is active, since there's then nothing to call out).
const activeFilterLabel = document.getElementById('active-filter-label');
function updateActiveFilterLabel(active) {
  ...
  if (!cat) { activeFilterLabel.hidden = true; return; }
  activeFilterLabel.innerHTML =
    '<span class="chip" style="background:' + cat.color + '"><svg viewBox="0 0 10 10"><use href="#icon-' + cat.icon + '"/></svg></span>' +
    cat.label;
...
(function () {
  ...
  function drawPixelOn(ctx, p, rotation) {
    ...
    // Drop shadow so pixels stay legible over whatever part of the map
    // (any colour) happens to be behind them now that there's no solid
    // banner background any more.
  ...
  function paintBackground(bg) {
    // DEV layout: no black backing behind the banner any more — the
    // pixel letters animate directly over the map, so clear to
    // transparent each frame instead of filling an opaque colour.
    surfaces.forEach(function (s) {
